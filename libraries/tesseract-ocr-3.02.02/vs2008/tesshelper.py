...
def getProjectfiles(libTessDir, libProjectFile, nTrimChars):
    ...
    for projectFile in projectFiles:
        ...
        if ext == ".c" or ext == ".cpp":
            ...
            print("unknown file type: {0}".format(projectFile))
...
def tessCompare(tessDir):
    ...
    print('Comparing VS2008 Project "{0}" with\n   "{1}"'.format(libProjectFile, tessAbsDir))
    ...
    print("{0} Extra files (in {1} but not in Project)".format(len(extraFiles), tessAbsDir))
    headerFiles = []
    sourceFiles = []
    ...
    print("")
    print("{0} new header file items written to {1}".format(len(headerFiles), NEWHEADERS_FILENAME))
    headerFiles.sort()
    with open(NEWHEADERS_FILENAME, "w") as f:
        for filename in headerFiles:
            f.write(fileNodeTemplate % filename)

    print("{0} new source file items written to {1}".format(len(sourceFiles), NEWSOURCES_FILENAME))
    sourceFiles.sort()
    ...
    print("{0} Dead files (in Project but not in {1}".format(len(deadFiles), tessAbsDir))
    sortedList = list(deadFiles)
    sortedList.sort()
    for filename in sortedList:
        print("   {0} ".fromat(filename))
...
def tessReport(tessDir):
    ...
    print('Summary stats for "{0}" library directories'.format(tessAbsDir))
    folderCounters = {}
    ...
    for folder in folders:
        ...
        print(" {0:5d} {1:3d} {2:3d} {3}".format(total, nHFiles, nCPPFiles, folder))
    print(" ----- --- ---")
    print(" {0:5d} {1:3d} {2:3d}".format(totalFiles, totalH, totalCPP))
    print(" ----- --- ---")
    print("Summary stats for VS2008 Project '%s'".format(libProjectFile))
    print(" {0:5d} {1}".format(len(projectHFiles), "Header files"))
    print(" {0:5d} {1}".format(len(projectCFiles), "Source files"))
    print(" {0:5d} {1}".format(len(projectRFiles), "Resource files"))
    print(" -----")
    print(" {0:5d}".format(len(projectHFiles) + len(projectCFiles) + len(projectRFiles)))
...
def copyIncludes(fileSet, description, tessDir, includeDir):
    ...
    print
    print('Copying libtesseract "%s" headers to %s'.format(description, includeDir))
    print
    ...
    for includeFile in sortedList:
        ...
        if os.path.isfile(filepath):
            ...
            print("Copied: %s".format(includeFile))
            count += 1
        else:
            print('***Error: "%s" doesn\'t exist"'.format(filepath))
            errList.append(filepath)
            
    print('%d header files successfully copied to "%s"'.format(count, includeDir))
    if len(errList):
        print("The following %d files were not copied:")
        for filepath in errList:
            print("   %s".format(filepath))
...
def tessCopy(tessDir, includeDir):
    ...
    if os.path.isfile(tessIncludeDir):
        print('Aborting: "%s" is a file not a directory.'.format(tessIncludeDir))
        return
    ...
    for rootDir, dirs, files in os.walk(vs2008AbsDir):
        for buildDir in ("LIB_Release", "LIB_Debug", "DLL_Release", "DLL_Debug"):
            if buildDir in dirs:
                ...
                if listOnly:
                    print("Would remove: %s".format(absBuildDir))
                else:
                    print("Removing: %s".format(absBuildDir))
        ...
        if rootDir == vs2008AbsDir:
            for file in files:
                if file.lower() not in ("tesseract.sln",
                    ...
                    if listOnly:
                        print("Would remove: %s".format(absPath))
                    else:
                        print("Removing: %s".format(absPath))
                        os.remove(absPath)
        else:
            for file in files:
